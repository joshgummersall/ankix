...
func TestBuildWordNote_BoldsWordInSentence(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			n := BuildYouTubeNote(nil, "Deck", "Title", "vid1", 5*time.Second, c.sentence, c.sel)
...
func TestBuildWordNote_HeaderLetsSameSentenceYieldDistinctCards(t *testing.T) {
	...
	n1 := BuildYouTubeNote(nil, "Deck", "Title", "vid1", 0, sentence, WordSelection{Start: 3, End: 7})
	n2 := BuildYouTubeNote(nil, "Deck", "Title", "vid1", 0, sentence, WordSelection{Start: 8, End: 13})
...
func TestBuildWordNote_BackHasGlossAndVideoLink(t *testing.T) {
	n := BuildYouTubeNote(nil, "Deck", "Title", "dQw4w9WgXcQ", 65*time.Second, "la casa vieja", WordSelection{Start: 3, End: 7, Gloss: "house"})
	back := n.Fields["Back"]
...
func TestBuildWordNote_NoLinkForNonYouTubeVideoID(t *testing.T) {
	...
	n := BuildYouTubeNote(nil, "Deck", "Title", "/tmp/sample.es.vtt", 65*time.Second, "la casa vieja", WordSelection{Start: 3, End: 7})
...
func TestBuildWordNote_VideoLinkClampsToZero(t *testing.T) {
	n := BuildYouTubeNote(nil, "Deck", "Title", "dQw4w9WgXcQ", 0, "la casa vieja", WordSelection{Start: 3, End: 7})
